...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text=dict_questions["question_pstart"], font=dict_features["large_font"]).place(relx=0.1,
                                                                                                               rely=0.1)
        popupMenu = ttk.OptionMenu(self, tkvar, "Options", *dict_answ_choices["choice_startpg"]).place(relx=0.1,
                                                                                                       rely=0.8)
        self.button0 = ttk.Button(self, text=dict_bttns['next_bttn'],
                                  command=lambda: controller.show_frame(PageOne)).place(relx=0.8, rely=0.8)
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageTwo)).place(
            relx=0.8, rely=0.8)
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label1 = tk.Label(self, text=ttl_qs, font=dict_features["small_font"], bg=dict_features['std_color']).grid(
            row=0, column=1)

        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 3, risk_tol_per_qs[
                                             'risk_pg_2'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageThree)).place(
            relx=0.8, rely=0.8)
...
class PageThree(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text=ttl_qs, font=dict_features['large_font'], bg=dict_features['std_color']).grid(row=0,
                                                                                                                  column=1)
        ...
        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageFour)).place(
            relx=0.8, rely=0.8)
...
class PageFour(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 6, risk_tol_per_qs[
                                             'risk_pg_4'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageFive)).place(
            relx=0.8, rely=0.8)
...
class PageFive(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text=ttl_qs, font=dict_features['medium_font'], bg=dict_features['std_color']).grid(
            row=0, column=1)

        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 5, risk_tol_per_qs[
                                             'risk_pg_5'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageSix)).place(
            relx=0.8, rely=0.8)
...
class PageSix(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text=ttl_qs, font=dict_features['small_font'], bg=dict_features['std_color']).grid(row=0,
                                                                                                                  column=1)

        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 3, risk_tol_per_qs[
                                             'risk_pg_6'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageSeven)).place(
            relx=0.8, rely=0.8)
...
class PageSeven(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 12, risk_tol_per_qs[
                                             'risk_pg_7'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageEight)).place(
            relx=0.8, rely=0.8)
...
class PageEight(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 6, risk_tol_per_qs[
                                             'risk_pg_8'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'], command=lambda: controller.show_frame(PageNine)).place(
            relx=0.8, rely=0.8)
...
class PageNine(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(len(ttl_ans)):
            checkbutton = tk.Radiobutton(self, text=ttl_ans[i], variable=var, value=i + 1,
                                         command=lambda: Complete_Interface.per_risk_tol(var, 3, risk_tol_per_qs[
                                             'risk_pg_9'])).grid(row=i + 1, column=1)

        self.grid_columnconfigure(1, weight=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'],
                            command=lambda: controller.show_frame(PageTen, parent, controller)).place(relx=0.8,
                                                                                                      rely=0.8)
...
class PageTen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label1 = tk.Label(self, text=prompt, font=dict_features['medium_font'], bg=dict_features['std_color']).grid(
            row=0, column=1)
        button = ttk.Button(self, text=dict_bttns['next_bttn'],
                            command=lambda: controller.show_frame(PageEleven)).place(relx=0.8, rely=0.8)
...
class Complete_Interface:
    ...
    def per_risk_tol(*args):
        ...
        print risk_tol_scale
        print "next"

        for possible_ans in range(1, num_of_qs_plus_1):
            if button_click.get() == possible_ans:
                risk_score = risk_tol_scale[
                    possible_ans - 1]  # Because our list start from 0, we subract 1. Thus, if the user clicked on the second button, the point given to risk is the in position 0 of the list
                print risk_score
