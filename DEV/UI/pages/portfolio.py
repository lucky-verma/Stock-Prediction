        ...
        if backTestProcess:
            if scenarios == 'No Shock' and re_balancing == 'Anually':
                ...
                with st.spinner(text='Portfolio Generation & Backtesting...'):
                    weights, values, stats = backTest(shock='no_shock', freq=252)
            elif scenarios == 'No Shock' and re_balancing == 'Monthly':
                st.spinner()
                with st.spinner(text='Portfolio Generation & Backtesting...'):
                    weights, values, stats = backTest(shock='no_shock', freq=21)
            elif scenarios == 'Shock' and re_balancing == 'Anually':
                st.spinner()
                with st.spinner(text='Portfolio Generation & Backtesting...'):
                    weights, values, stats = backTest(shock='shock', freq=252)
            else:
                st.spinner()
                with st.spinner(text='Portfolio Generation & Backtesting...'):
                    weights, values, stats = backTest(shock='shock', freq=21)
            ...
            if prev_vars[-3] != "Aggressive":
                ...
                font = ['#ffffff', '#008B00', '#004F00', '#008B00', '#008B00', '#004F00', '#004F00', '#004F00',
                        '#004F00']
                ...
                fig = go.Figure(data=[go.Bar(x=temp_bar_x[1:], y=temp_bar_y[1:],
                ...
                # writer = pd.ExcelWriter('Backtesting_Rahul.xlsx', engine='xlsxwriter')
